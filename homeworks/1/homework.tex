...
We installed the corresponding libraries in our machines.
 
\subsection{Part (c)} 
After running \textit{hello\_world.f90}, we succeded in getting the message
\begin{verbatim}
...
a = \exp(5+0.001 b),
\end{equation}
where $b$ is a random number obtained at each iteration (see code below). As we increase the number of processors (\textit{np}), 
